...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, onClick }) => {
  return (
    <div className="group flex flex-col gap-6 cursor-pointer" onClick={() => onClick(service)}>
      ...
      <div className="text-center">
        <h3 className="text-2xl font-serif font-medium text-[#2C2A26] mb-1 group-hover:opacity-70 transition-opacity">{service.name}</h3>
        <p className="text-sm font-light text-[#5D5A53] mb-3 tracking-wide">{service.category}</p>
        <span className="text-sm font-medium text-[#2C2A26] block">Starting from ${service.price}</span>
      </div>
    </div>
  );
};

...
const InquiryDrawer: React.FC<InquiryDrawerProps> = ({ isOpen, onClose, items, onRemoveItem, onCheckout }) => {
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#D6D1C7]">
          <h2 className="text-xl font-serif text-[#2C2A26]">Project Brief ({items.length})</h2>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
              ...
              <p className="font-light text-[#5D5A53]">Your brief is empty.</p>
            </div>
          ) : (
            items.map((item, idx) => (
        ...
        <div className="p-6 border-t border-[#D6D1C7] bg-[#EBE7DE]/30">
          <div className="flex justify-between items-center mb-6">
            <span className="text-sm font-medium uppercase tracking-widest text-[#5D5A53]">Estimated Total</span>
            <span className="text-xl font-serif text-[#2C2A26]">${total}</span>
          </div>
          <p className="text-xs text-[#A8A29E] mb-6 text-center">Final pricing will be discussed during our consultation.</p>
          <button 
            onClick={onCheckout}
            disabled={items.length === 0}
            className="w-full py-4 bg-[#EF4444] text-white uppercase tracking-widest text-sm font-bold hover:bg-[#DC2626] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Start Inquiry
          </button>
        </div>
